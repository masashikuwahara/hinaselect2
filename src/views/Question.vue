<template>
  <div class="p-4 sm:p-6 max-w-md sm:max-w-xl mx-auto">
    <h2 class="text-xl sm:text-2xl font-bold text-center mb-6">質問 {{ currentIndex + 1 }} / {{ questions.length }}</h2>
    <transition name="fade" mode="out-in">
      <div :key="currentIndex" class="bg-white p-4 sm:p-6 rounded-xl shadow-lg text-center">
        <p class="text-base sm:text-lg font-semibold mb-4">{{ currentQuestion.text }}</p>
        <div class="space-y-3">
          <button
